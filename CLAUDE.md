...
Iterate as many PI's as agreed in 1d above. In each PI:
  ...
  2c. Brief to Product Team: With Product Vision in mind and using learnings (in team_rules_product_team.md), refine (analyze and clarify) features in scope for this PI. Maintained in features.md and possibly backlog.md. Features analyzed/clarified change status to Backlog (because they are ready for IT Dev Team refinement). Keep Product Team alive for IT Dev team refinement of features.
    **Matrix refinement (added PI 16):** For each feature in scope whose "Pages touched" field names one or more files in `docs/quality/matrices/`, the BA opens that matrix, reviews every cell for the 7 personas in `docs/quality/personas.md`, and updates cells the feature changes. UX Designer reviews for cross-page coherence; Architect signs off on state-transition implementability. Cells flagged ⚠ DEFECT feed fix scope (same PI if High/Medium, deferred feature if Low). See `team_rules_product_team.md` Rule 4.
...
3b: Start Business Test Team with as many BA/test agents you think fit for them to make sure that 3a doesn't happen. **Persona-journey framing (added PI 16):** BA work is driven by `docs/quality/personas.md` × `docs/quality/matrices/*.md`, not by the PI's feature list. For each persona, walk every reachable page and verify the matrix cells match reality. Apply the 5-point consistency checklist on every screen (see `team_rules_test_team.md` Rule 3). Pre-condition: persona smoke (F-046 `frontend/tests/smoke/personas.spec.ts`) must be green post-deploy before BA starts.
...
| `docs/product/pi3–pi??/` | Historical PI planning + BA verification | Archive — read-only |
