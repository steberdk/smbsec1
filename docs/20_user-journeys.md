...
### Public
Home → Why It Matters → High Impact Preview → Trust & Privacy → Signup

### Authenticated
Login → Create Organization (first time) → Risk Snapshot → Checklist Workspace → Fix Detail → Dashboard → Invite Users → GDPR Hub

### Checklist Workspace layout
- Progress bar (top) / Category navigation (left) / Current item (center) / Primary action (bottom)
- States: Loading | In-progress | Completed | Skipped | Not sure

### Dashboard
Displays: current score, previous score, team completion, last reassessment date.
Actions: Run reassessment / Download report / Invite users.

### GDPR Hub
Displays: stored org data, employee data, assessment history, phishing campaign history.
Actions: Export data / Delete single assessment / Delete organization (hard delete).
