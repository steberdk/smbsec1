-- Migration 027: F-060 — Fix `digest()` resolution on Supabase-hosted Postgres.
--
-- Problem. Migration 024 declared `CREATE EXTENSION IF NOT EXISTS pgcrypto;`
-- at the top and used `encode(digest(x, 'sha256'), 'hex')` inside
-- `smbsec1.delete_member_with_data`. On Supabase, `pgcrypto` is installed in
-- the `extensions` schema (not `public`), and the function's
-- `SET search_path = smbsec1, public` does NOT include `extensions` — so at
-- runtime `digest(text, unknown) does not exist`. The function caught it via
-- its EXCEPTION WHEN OTHERS block and returned
-- `{"success": false, "error": "function digest(text, unknown) does not exist"}`.
-- Effect: every real member-deletion + pending-invite revoke+delete path
-- returned "success:false" in the RPC payload, which the API surfaced as a
-- generic 500 to the UI. F-049 AC-1, F-033 AC-1 blocked on PROD.
--
-- Fix. Replace both `digest()` calls with native-Postgres equivalents that do
-- not require the pgcrypto extension:
--   encode(digest(text, 'sha256'), 'hex')
--     becomes
--   encode(sha256(text::bytea), 'hex')
--
-- `sha256()` is a built-in function in PostgreSQL 14+ (Supabase runs PG 15 at
-- time of writing), so no extension or search_path tweak is needed. This is
-- the portable fix.
--
-- This migration is idempotent — `CREATE OR REPLACE FUNCTION` re-specifies
-- the function in full. Apply in Supabase SQL editor after migrations 024+025.
...
BEGIN
  ...
  -- 4. Compute hashes — native PG 14+ sha256, no pgcrypto dependency.
  v_email_sha := encode(sha256(v_target_email_lc::bytea), 'hex');
  IF v_target_user_id IS NOT NULL THEN
    v_user_sha := encode(sha256(v_target_user_id::text::bytea), 'hex');
  ELSE
    v_user_sha := NULL;
  END IF;

  -- 5. Cascade deletes inside the implicit transaction.
  -- 5a. assessment_responses
  IF v_target_user_id IS NOT NULL THEN
  ...
  -- 6. Anonymise residual audit_logs rows.
  IF v_target_user_id IS NOT NULL THEN
  ...
  -- 7. Insert member_removed audit row (hashed identifiers only).
